...
$query = "SELECT courses.link,assigncourse.date_created, courses.title,assigncourse.department,assigncourse.userId, assigncourse.id as assign_id, courses.filename FROM assigncourse INNER JOIN courses ON courses.id = assigncourse.courseId INNER JOIN coursecategory ON coursecategory.id = courses.id  WHERE assigncourse.companyId = '".$_SESSION['user']['companyId']."' ORDER BY courses.id DESC";
  ...
  $query = "SELECT coursestest.id, coursestest.testexpirationdate,coursestest.date_created, coursestest.testname,coursestest.questions, users.name, users.fname  FROM coursestest INNER JOIN users ON coursestest.createdBy = users.id WHERE coursestest.companyId = '".$_SESSION['user']['companyId']."' ORDER BY coursestest.id DESC";
                        ...
                        <div class="question_page">   
                          ...
                          <div class="row">
                            <div class="col-md-8 col-sm-12 col-xs-12 col-md-offset-2"> 
                             ...
                             <input style="margin-top: 20px" class="form-control" type="text" placeholder="Correct Option (A, B, C, D, E)" id="answer" name="">                   
                            </div>
                          </div>
                        </div>
                        ...
                        <tbody>
                          ...
                          <tr class="pointer">
                            ...
                            <td class=" ">
                              ...
                              <a href ="downloadfile.php/?to=view&filename=<?=$courses[$h]['filename']?>" id=""
                                name='' value=""
                            ...
                            <td class="edit" id="edit<?=$h?>" onclick = "Deleteitem(<?=$courses[$h]['assign_id']?>)" data-toggle="modal" data-target="#editModal" courseid="<?=$courses[$h]['courseid']?>" categoryId = "<?=$courses[$h]['categoryId']?>" title = "<?=$courses[$h]['title']?>" link = "<?=$courses[$h]['link']?>"><span class="fa fa-close"></span></td>
                            <th class="column-title"><a href="participantlist.php/?courseid=<?=base64_encode($courses[$h]['id'])?>&department=<?=$courses[$h]['department']?>&userId=<?=$courses[$h]['userId']?>" class="btn btn-sm btn-success">Details</a> </th>
                          </tr>
                           <?php }?>
                        </tbody>
                      </table>
