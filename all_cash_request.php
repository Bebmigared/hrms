...
$query = "SELECT users.name, users.department,cash_request.staff_id,cash_request.admin_id,cash_request.cash_id,cash_request.purpose,cash_request.justification, cash_request.date_created, cash_request.amount,cash_request.id FROM cash_request INNER JOIN users ON users.id = cash_request.staff_id WHERE cash_request.admin_id = '".$_SESSION['user']['id']."' ORDER BY cash_request.id DESC";
 }else{
  $query = "SELECT users.name, users.department,cash_request.staff_id,cash_request.cash_id,cash_request.admin_id,cash_request.purpose,cash_request.justification, cash_request.date_created, cash_request.amount,cash_request.id FROM cash_request INNER JOIN users ON users.id = cash_request.staff_id WHERE cash_request.admin_id = '".$_SESSION['user']['admin_id']."' ORDER BY cash_request.id DESC";
 }
                      ...
                      <table class="table table-striped jambo_table bulk_action">
                        ...
                        <tbody>
                          ...
                          <tr class="pointer">
                            ...
                            <td class=""><?=$data_cash[$h]['cash_id']?></td>
                            <td class=""><?=$data_cash[$h]['name']?></td>
                            <td class=" ">
                             
                               <?php 
                              $query = "SELECT * FROM departments WHERE id = '".$data_cash[$h]['department']."'";
                              $result = mysqli_query($conn, $query);
                              if(mysqli_num_rows($result)> 0){
                                  while($row = mysqli_fetch_assoc($result)) {
                                    echo $row['dept'];
                            ...
                            <td class=" ">
                              <!-- <?=$data_cash[$h]['purpose']?> -->
                              <?php
                               $query = "SELECT * FROM cash_category WHERE id = '".$data_cash[$h]['purpose']."'";
                                $result = mysqli_query($conn, $query);
                                if(mysqli_num_rows($result)> 0){
                                    while($row = mysqli_fetch_assoc($result)) {
                                      echo $row['category'];
                                   ...
                                   echo $data_cash[$h]['purpose'];
                                }
                                ?> 
                            </td>
                            <td class=" "><?=number_format($data_cash[$h]['amount'])?></td>
