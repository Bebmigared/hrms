 ...
 if(isset($_POST['submit'])){
    ...
    //echo $open_for;
 	if($opening_date != '' && $closing_date != '' && $open_for != ''){
      $query = "SELECT * FROM open_information_portal WHERE companyId = '".$_SESSION['user']['companyId']."' AND open_for = '".$open_for."'";
	  ...
	  if(mysqli_num_rows($res)> 0){
		$sql = "UPDATE open_information_portal SET opening_date = '".$opening_date."', closing_date = '".$closing_date."', open_for = '".$open_for."', date_created = '".date('Y-m-d')."' WHERE admin_id = '".$_SESSION['user']['id']."' AND open_for = '".$open_for."'";
	     if (mysqli_query($conn, $sql)) {
	        ...
	        header("Location: open_portal.php");
	    } else {
     	  ...
     	  insert_data($conn,$opening_date,$closing_date, $open_for, $_SESSION['user']['id'], $_SESSION['user']['companyId']);
          header("Location: open_portal.php");
     }		
     
   }
 }
 function insert_data($conn, $opening_date,$closing_date, $open_for, $_id, $companyId){
 	$sql = "INSERT INTO open_information_portal (opening_date, closing_date, open_for, admin_id, date_created, companyId)
          VALUES ('".$opening_date."', '".$closing_date."', '".$open_for."','".$_id."', '".date('Y-m-d')."', '".$companyId."')";
            if (mysqli_query($conn, $sql)) {
               $_SESSION['msg'] = "Employee Information will be open between ".$opening_date." and ".$closing_date."";
            }else {
               $_SESSION['msg'] = "Error while update account, please try again later";
